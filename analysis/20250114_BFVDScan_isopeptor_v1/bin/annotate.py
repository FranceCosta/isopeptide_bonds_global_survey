...
""" 
    ...
    `sbatch --job-name="annotate_afdb_scan" -t 00:30:00 --mem 8GB -e log/annotate_%j.err \
            -o log/annotate_%j.out --wrap="python3 bin/annotate.py"`
    Francesco Costa 2024-01-15 fcosta@ebi.ac.uk
...
CACHE_DOMAIN = "tmp/cached_domains.csv"
CACHE_TAX = "tmp/cached_taxonomy.csv"
CACHE_NO_HMM_TABLE = "tmp/cached_no_hmm.csv"
RESULT = "/hps/nobackup/agb/research/francesco/tmp/jessBFVD_v1/jess_out.csv"
DATE = datetime.today().strftime('%Y%m%d')
OUTPUT_TABLE = os.path.join("output", f"{DATE}_vbdf_scan.csv")
...
def main():
    ...
    df = pd.read_csv(RESULT, names=columns)
    df["protein_name"] = df["protein_name"].apply(lambda x: x.split("_")[0])
    print(df)
    ...
    if os.path.isfile(CACHE_HMM_TABLE) == False or not USE_CACHE:
        df = hmm_domain(df)
        ...
        df = pd.read_csv(CACHE_HMM_TABLE)
...
def clanacc_to_id(clan_accs:list) -> pd.DataFrame:
    ...
    query = f"SELECT clan_acc, clan_id \
