...
AROMATICS = {
    "PHE": {
        'full': ["CG", "CD1", "CD2", "CE1", "CE2", "CZ"]
    }, 
    "TYR": {
        'full': ["CG", "CD1", "CD2", "CE1", "CE2", "CZ"],
    }, 
    "TRP": {
        'full': ["CD2", "CE2", "CE3", "CZ3", "CH2", "CZ2", 
                "CG", "CD1", "NE1"], # both rings
        'ring1': ["CD2", "CE2", "CE3", "CZ3", "CH2", "CZ2"],
        'ring2': ["CD2", "CE2", "CG", "CD1", "NE1"]
    }
}
...
def main():
    ...
    df["aro_plane-isopep_NZ_plane_angle"] = df.apply(lambda x: n_isopep_aro_plane_angle(x), axis=1)
...
def get_aro_params(df, dist_threshold=10) -> pd.DataFrame:
    """
        ...
        Get data asscoiated to aro rings: lenght, angle between isopeptide bond NZ atom and ring plane;
    ...
    for index, row in df.iterrows():
        ...
        structure = structure[(structure.hetero==False) & (structure.chain_id==chain)]
        # Get isopep atoms
        isopep_atoms = structure[((structure.res_id == r1) & (structure.atom_name == "NZ")) | \
            ((structure.res_id == r3) & (np.isin(structure.atom_name, ["CG", "OD1", "OD2"])))]
        ...
        nz_plane_atoms = isopep_atoms[((isopep_atoms.res_id==r1) & \
            (np.isin(isopep_atoms.atom_name, ["NZ", "CE"]))) | \
        ...
        def get_distance(aro_atoms, lys_nz):
            """Get min distance with aro ring and center"""
            distance_ring = np.min(aro_atoms.distance)
            nz = lys_nz.coord[0]

            if aro_atoms[0].res_name == "TRP":
                ring_1 = aro_atoms[np.isin(aro_atoms.atom_name, AROMATICS['TRP']["ring1"])]
                ring_2 = aro_atoms[np.isin(aro_atoms.atom_name, AROMATICS['TRP']["ring1"])]
            ...
            return [ring_center, distance_ring, distance_center, min(distance_ring, distance_center)]

        # Order aro rings by distance with ring (consider min ring and center distance)
        aro_tuples = []
        for res_id in aro_residues_within_threshold:
            aro_atoms = aro_rings[aro_rings.res_id==res_id]
            ring_center, distance_ring, distance_center, distance = get_distance(aro_atoms, lys_nz)
            aro_tuples.append((aro_atoms, ring_center, distance_ring, distance_center, distance))
        
        # Sort by minimum distance 
        aro_tuples = sorted(aro_tuples, key=lambda x: x[4])
        
        # Iterate until a cap is detected otherwise return the first aro residue
        aro_output = None
        for aro_data in aro_tuples:
            aro_atoms, ring_center, distance_ring, distance_center, distance = aro_data
            nz = lys_nz.coord[0]
            res_id = aro_atoms[0].res_id
            aro_cap = False

            # Compute normal vector and normalize
            v1 = aro_atoms[1].coord - aro_atoms[0].coord
            v2 = aro_atoms[2].coord - aro_atoms[0].coord
            # Calculate the normal vector to the ring plane using the cross-product
            ...
            #if row["PDB code"] == "3gle":
            #    print(row["PDB code"], aro_tuples)

            if distance_center < dist_cutoff and ((angle_degrees < angle_cutoffs[0]) or (angle_degrees > angle_cutoffs[1])):
                aro_cap = True
    ...
    return pd.DataFrame(outlist, 
            ...
            "aro_res_id", "aro_res_name", "distance_to_aro_ring", "distance_to_aro_center", "min_distance_to_aro",
...
def n_isopep_aro_plane_angle(row) -> float:
    ...
    aro_atoms = struc.array([atom for atom in structure if atom.res_id == aro_res_id and \
                                                        atom.atom_name in AROMATICS[atom.res_name]['full'][:3]])
